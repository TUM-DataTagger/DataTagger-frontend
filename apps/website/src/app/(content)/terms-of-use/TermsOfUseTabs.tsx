...
export function TermsOfUseTabs(
	props: PropsWithChildren<{
		readonly textDE: string;
		readonly textEN: string;
	}>,
) {
	const [termsOfUseTab, setTermsOfUseTab] = useState('english');

	return (
		<Tabs className="gap-0" onSelectionChange={(key) => setTermsOfUseTab(String(key))} selectedKey={termsOfUseTab}>
			<TabList className="h-8 place-content-center gap-0 border-0">
				<Tab
					...
					id="english"
				>
					{termsOfUseTab === 'english' ? <CheckIcon aria-hidden size={18} strokeWidth={1.5} /> : null}
					English (EN)
				</Tab>
				<Tab
					...
					id="german"
				>
					{termsOfUseTab === 'german' ? <CheckIcon aria-hidden size={18} strokeWidth={1.5} /> : null}
					German (DE)
				</Tab>
			</TabList>
