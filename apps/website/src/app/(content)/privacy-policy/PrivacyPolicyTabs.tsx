...
export function PrivacyPolicyTabs(
	...
	const [privacyPolicyTab, setPrivacyPolicyTab] = useState('english');

	return (
		<Tabs
			className="gap-0"
			onSelectionChange={(key) => setPrivacyPolicyTab(String(key))}
			selectedKey={privacyPolicyTab}
		>
			<TabList className="h-8 place-content-center gap-0 border-0">
				<Tab
					...
					id="english"
				>
					{privacyPolicyTab === 'english' ? <CheckIcon aria-hidden size={18} strokeWidth={1.5} /> : null}
					English (EN)
				</Tab>
				<Tab
					...
					id="german"
				>
					{privacyPolicyTab === 'german' ? <CheckIcon aria-hidden size={18} strokeWidth={1.5} /> : null}
					German (DE)
				</Tab>
			</TabList>
