...
import { ProjectMetadataTemplatesCreateTable } from './ProjectMetadataTemplatesCreateTable';
...
export function ProjectMetadataTemplatesTable(props: { readonly data?: ProjectMetadataTemplate[] | undefined }) {
	...
	return (
		<div className="flex flex-col">
			<ProjectMetadataTemplatesCreateTable
			...
			/>
			<Table aria-label="Project metadata templates">
				...
				<TableBody items={props.data ?? []}>
					{(item) => (
						<TableRow href={`/metadata-templates/${item.pk}`} id={item.pk}>
							...
							<TableCell
								style={{ width: '50px' }}
								textValue={item.pk === projectData.metadata_template?.pk ? 'Checked' : ''}
							>
