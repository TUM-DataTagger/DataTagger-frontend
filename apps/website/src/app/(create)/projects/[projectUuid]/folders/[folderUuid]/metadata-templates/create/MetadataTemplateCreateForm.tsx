...
export function MetadataTemplateCreateForm({
	...
	folderUuid,
	...props
}: ComponentProps<'form'> & { readonly folderUuid: string; readonly projectUuid: string }) {
	...
	const {
		...
		defaultValues: {
			name: '',
			assigned_to_content_type: 'folders.folder',
			assigned_to_object_id: folderUuid,
	...
	const { mutate } = useMutation({
		mutationFn: async (input: MetadataTemplateCreateInput) => {
			const payload = {
				name: input.name,
				assigned_to_content_type: 'folders.folder' as const,
				assigned_to_object_id: folderUuid,
		...
		async onSuccess() {
			...
			await queryClient.invalidateQueries({ queryKey: ['folder', folderUuid, 'metadata-templates'] });
			await queryClient.invalidateQueries({
			...
			router.push(`/projects/${projectUuid}/folders/${folderUuid}/metadata-templates`);
